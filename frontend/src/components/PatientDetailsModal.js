  ...
  const getFactors = () => {
    ...
    if (prediction && prediction.factors) {
      ...
      if (Array.isArray(patient.risk_factors)) {
        ...
        try { rawFactors = JSON.parse(patient.risk_factors); } catch (e) { rawFactors = []; }
      }
    }
    ...
    <div className="modal-overlay">
      <div className="modal-content">
        ...
        <div className="modal-body custom-scrollbar">
          ...
          {activeTab === 'overview' && (
            <>
              <div className="info-grid">
                <div className="info-item"><label>Phone Contact</label><p>{patient.phone_number || 'N/A'}</p></div>
                <div className="info-item"><label>Months on ART</label><p>{patient.months_on_art || '0'}</p></div>
                <div className="info-item"><label>WHO Clinical Stage</label><p>Stage {patient.who_clinical_stage || 'N/A'}</p></div>
                <div className="info-item"><label>Regimen</label><p>{patient.arv_regimen || 'Standard'}</p></div>
                <div className="info-item"><label>Marital Status</label><p>{patient.marital_status || 'N/A'}</p></div>
                <div className="info-item"><label>Treatment Supporter</label><p>{patient.treatment_supporter ? 'Yes' : 'No'}</p></div>
                <div className="info-item"><label>Chronic Score</label><p>{patient.chronic_score || '0'}</p></div>
                <div className="info-item"><label>TB Co-infection</label><p>{patient.tb_flag ? 'Yes' : 'No'}</p></div>
                {patient.gender === 'F' && (
                  <div className="info-item"><label>Pregnancy Status</label><p>{patient.pregnancy_flag ? 'Pregnant' : 'Not Pregnant'}</p></div>
                )}
                <div className="info-item"><label>Status</label><p>{patient.is_active ? 'Active' : 'Inactive'}</p></div>
              </div>
              ...
              <div className="risk-section">
                <div className="risk-header">
                  ...
                  <span className="risk-badge" style={{ backgroundColor: riskColor }}>{activeLabel.toUpperCase()} RISK</span>
                </div>

                <div style={{ marginBottom: '1rem' }}>
                  <button className="btn-primary" onClick={runPrediction} disabled={predicting} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.85rem' }}>
                    {predicting ? <><Loader size={14} className="spin" /> Running XGBoost Model...</> : <><Brain size={14} /> Run AI Risk Predictor</>}
                  </button>
                  {activeSource && (
                    <span style={{ marginTop: '0.4rem', display: 'inline-block', fontSize: '0.72rem', padding: '2px 8px', borderRadius: '999px', backgroundColor: activeSource === 'ml_model' ? '#d1fae5' : '#fef3c7', color: activeSource === 'ml_model' ? '#065f46' : '#92400e' }}>
                ...
                <div className="risk-meter-container">
                  <div className="risk-score-label"><span>Predicted Default Probability</span><span style={{ color: riskColor, fontWeight: 'bold' }}>{activeScore}%</span></div>
                ...
                <div className="risk-factors-box">
                  ...
                  {activeFactors.length > 0 ? (
                    <ul className="factors-list">{activeFactors.map((factor, i) => <li key={i}>{factor}</li>)}</ul>
                  ) : (
                    <p className="no-risk">{prediction ? 'No significant risk factors detected.' : 'Click Run AI Risk Predictor to analyse this patient.'}</p>
                  )}
                </div>
              </div>
            </>
          ...
          {/* TAB 3: ADD LAB RESULTS WORKSPACE VIEW */}
          {activeTab === 'labs' && (
            <div className="labs-tab-content" style={{ padding: '1rem' }}>
              <h3 style={{ marginBottom: '0.5rem' }}>Enter Clinical Lab Data</h3>
              <p style={{ color: '#6b7280', fontSize: '0.85rem', marginBottom: '1.5rem' }}>
                Input current diagnostics below. Saving automatically syncs values and recalculates AI adherence risk factors.
              </p>
              <LabEntryForm patientId={patient.patient_id} onSuccess={() => setActiveTab('overview')} />
            </div>
          )}
        </div>
